...
(defn library-dependencies
  ...
  (concat
    ...
    ; ensure that goog-style dependencies from the classpath are brought in
    ; load-library is unfortunately responsible for bringing in both classpath
    ; and filesystem resources; passing an empty path (prefix) is the simplest
    ; way to confine the result of this call to the classpath deps
    (mapcat load-library (conj (set libs) ""))
